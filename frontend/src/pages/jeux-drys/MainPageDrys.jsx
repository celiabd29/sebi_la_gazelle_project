...
const MainPage = () => {
  ...
  // ✅ Clic utilisateur déclenche startSound()
  useEffect(() => {
    const handleClick = () => {
      if (soundOn) {
        startSound();
  ...
  useEffect(() => {
  if (!musicOn) return; // 👈 ici on regarde musicOn
  ...
  // ✅ Joue mainVoice une fois, puis secondAudioFile en boucle toutes les 30s après 5s
  useEffect(() => {
    if (!soundOn) return;
